...
osMessageQId coreQueueHandle;
osMessageQId modemQueueHandle;
...
int main(void)
{
  ...
  osTimerStart(timer1Handle, 1);
  /* USER CODE END RTOS_TIMERS */

  /* Create the queue(s) */
  /* definition and creation of mainQueue */
  osMessageQDef(mainQueue, 50, void *);
  mainQueueHandle = osMessageCreate(osMessageQ(mainQueue), NULL);

  /* definition and creation of coreQueue */
  osMessageQDef(coreQueue, 20, uint8_t);
  coreQueueHandle = osMessageCreate(osMessageQ(coreQueue), NULL);

  /* definition and creation of modemQueue */
  osMessageQDef(modemQueue, 16, void *);
  modemQueueHandle = osMessageCreate(osMessageQ(modemQueue), NULL);
  ...
  osThreadDef(mainTask, app_main, osPriorityNormal, 0, 1024);
  mainTaskHandle = osThreadCreate(osThread(mainTask), NULL);

  /* definition and creation of coreTask */
  osThreadDef(coreTask, app_core, osPriorityRealtime, 0, 512);
  coreTaskHandle = osThreadCreate(osThread(coreTask), NULL);

  /* definition and creation of modemTask */
  osThreadDef(modemTask, app_modem, osPriorityIdle, 0, 512);
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOC, LED_0_Pin|LED_1_Pin|LED_2_Pin|LED_3_Pin 
                          |LED_4_Pin|LED_5_Pin|LED_6_Pin|LED_7_Pin, GPIO_PIN_SET);
  ...
  /*Configure GPIO pins : FPGA_START_Pin FPGA_RST_Pin */
  GPIO_InitStruct.Pin = FPGA_START_Pin|FPGA_RST_Pin;
...
/* USER CODE BEGIN Header_app_core */
/**
* @brief Function implementing the coreTask thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_app_core */
__weak void app_core(void const * argument)
{
  /* USER CODE BEGIN app_core */
  /* Infinite loop */
  for(;;)
  {
    osDelay(1);
  }
  /* USER CODE END app_core */
}

/* USER CODE BEGIN Header_app_modem */
/**
* @brief Function implementing the modemTask thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_app_modem */
__weak void app_modem(void const * argument)
{
  /* USER CODE BEGIN app_modem */
  /* Infinite loop */
  for(;;)
  {
    osDelay(1);
  }
  /* USER CODE END app_modem */
}

/* timer1_cb function */
__weak void timer1_cb(void const * argument)
{
